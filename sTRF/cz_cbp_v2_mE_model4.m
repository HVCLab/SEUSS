...
fullinputFolder = fullfile(inputFolder,'model4');
...
GAdiff{1}.comparison = {'main eff. RHYTHM mag (reg vs irr)'};

GAdiff{2} = ft_math(cfg, GAc{4}, GAc{5});
GAdiff{2}.comparison = {'main eff. of RHYTHM bin (reg vs irr)'};


%% imagesc of GAdiff
% 
for cond = 1:length(GAdiff)
subplot(1,2,cond);imagesc(GAdiff{cond}.time,1:size(GAdiff{cond}.avg,1),GAdiff{cond}.avg);title(GAdiff{cond}.comparison{1});colorbar;
end
...
load(fullfile(fullinputFolder,'chanClusters.mat'))
clstrNmb = -1;
clusterX = clusters{2}(clusters{2}(:,2)==clstrNmb,:);
chan = clusterX(:,1);
clustalfa = [0.05];
...
for num_var_idx = 1:numel(clustalfa)
    ...
    cfg.minnbchan = 0;if multi cfg.minnbchan = 3; end
    ...
    statC{1}.comparison = {'main eff. RHYTHM mag (reg vs irr)'};

    statC{2} = ft_timelockstatistics(cfg, allDataC{4}{:}, allDataC{5}{:});
    statC{2}.comparison = {'main eff. of RHYTHM bin (reg vs irr)'};
...
%% save 
filename = 'cbp_uni_mtrClustr-1El_091224_-100to600.mat';
%  filename = 'cbp_multi_mtrAllEl_091224_0to600_minbchan3.mat';
...
filename = 'cbp_uni_mtrClustr-1El_091224_-100to600.mat';
close all
cz_plot_cbp_model34(inputFolder,filename)
