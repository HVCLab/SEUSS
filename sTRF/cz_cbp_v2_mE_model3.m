...
fullinputFolder = fullfile(inputFolder,'model3');
...
GAdiff{1}.comparison = {'main eff. of STRESS mag (uns vs str)'};

GAdiff{2} = ft_math(cfg, GAc{4}, GAc{5});
GAdiff{2}.comparison = {'main eff. of STRESS bin (uns vs str)'};
...
for num_var_idx = 1:numel(clustalfa)
    ...
    cfg.latency = [-0.1 0.6]; if multi cfg.latency = [0 600];end% time interval over which the experimental conditions must be compared (in seconds)

    % Step 3: Perform the statistical comparison

    statC{1} = ft_timelockstatistics(cfg, allDataC{2}{:}, allDataC{3}{:});
    statC{1}.comparison = {'main eff. of STRESS mag (uns vs str)'};

    statC{2} = ft_timelockstatistics(cfg, allDataC{4}{:}, allDataC{5}{:});
    statC{2}.comparison = {'main eff. of STRESS bin (uns vs str)'};
